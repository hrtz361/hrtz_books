...
class Person {
    ...
    public:
        ...
        void Input();
        void Display();
        // [FIX 15] Thêm readFromFile vào Person d? Reader có th? g?i khi load d? li?u
...
class Reader : public Person {
    ...
    public:
        ...
        // [FIX 13] Thêm getter cho registrationDate và expirationDate (c?n cho saveToFile)
        ...
        // [FIX 14] Thêm readFromFile cho Reader
...
void Reader::Display() {
    ...
    cout << "\nMa nguoi doc: "    << readerID
         ...
         // [FIX 1] Them dau ngoac vao toan tu ternary de tranh loi precedence voi <<
...
class Book {
    ...
    public:
        ...
        // [FIX 16] Thêm readFromFile thu?n ?o d? các l?p con implement
        virtual void readFromFile(ifstream& f) {
            getline(f, bookID,     ',');
            getline(f, title,      ',');
            getline(f, author,     ',');
...
class PrintedBook : public Book {
    ...
    public:
        ...
        // [FIX 3] Thêm weight, status, shelfLocation vào writeToFile (tru?c dây b? thi?u)
        void writeToFile(ofstream& f) override {
            f << 1 << ",";
            Book::writeToFile(f);
            f << pageCount << "," << weight << "," << shelfLocation << "," << status << "\n";
        }
        // [FIX 4] Thêm readFromFile cho PrintedBook
        void readFromFile(ifstream& f) override {
            Book::readFromFile(f);
            string pageStr;
            getline(f, pageStr, ',');
...
class Ebook : public Book {
    ...
    public:
        ...
        // [FIX 4] Thêm readFromFile cho Ebook
        void readFromFile(ifstream& f) override {
            Book::readFromFile(f);
            getline(f, fileSize,     ',');
...
public:
    ...
    // [FIX 12] Thêm getDueDate() (c?n cho ThongKeBaoCao::DanhSachQuaHan)
...
class QuanLySach {
    ...
    public:
        ...
        // [FIX 5] loadFromFile gi? d?c d? d? li?u t?ng quy?n sách (tru?c ch? t?o object r?ng)
        void loadFromFile() {
            ifstream f("sach.txt");
            if (!f.is_open()) return;
            danhSachSach.clear();
        ...
        // [FIX 8] Thêm sub-menu qu?n lý sách
        void MenuQuanLySach() {
            int sub;
            do {
                cout << "\n--- QUAN LY SACH ---\n"
...
class QuanLyDocGia {
    ...
    public:
        ...
        // [FIX 6] saveToFile luu d?y d? t?t c? các tru?ng (tru?c ch? luu 2 tru?ng)
        void saveToFile() {
        ...
        // [FIX 7] loadFromFile gi? d?c và khôi ph?c d?y d? danh sách (tru?c d? tr?ng)
        void loadFromFile() {
        ...
        // [FIX 8] Thêm sub-menu qu?n lý d?c gi?
...
// ===================== CLASS QUANLYMUONTRA =====================
// [FIX 2] Chuy?n LapPhieuMuon, CapNhatNgayTra, XuLyMatSach vào bên trong class
//         (tru?c dây khai báo ngoài class nhung truy c?p danhSachPhieu private => l?i biên d?ch)
class QuanLyMuonTra {
private:
...
public:
    ...
    void LapPhieuMuon(QuanLyDocGia& qlDG, QuanLySach& qlSach) {
        ...
        // [FIX 11] C?p nh?t borrowCount khi mu?n thành công (tru?c b? b? sót)
    ...
    // [FIX 8] Thêm sub-menu qu?n lý mu?n tr?
...
public:
    ...
    // [FIX 9] Tri?n khai vòng l?p tìm phi?u quá h?n (tru?c d? tr?ng)
    void DanhSachQuaHan(QuanLyMuonTra& qlMT, QuanLyDocGia& qlDG) {
        cout << "\n--- DOC GIA NO SACH QUA HAN ---\n";
        // Ngày hi?n t?i: 13/05/2026 (t?ng ngày tính t? nam 1)
        // Thay b?ng hàm l?y ngày th?c t? n?u c?n
    ...
    // [FIX 10] Tri?n khai tính t?ng ti?n ph?t (tru?c d? tr?ng)
